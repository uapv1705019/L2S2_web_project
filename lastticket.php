...
<html>
	<head>
	    <meta charset="utf-8" />
		<title>Projet : derniers tickets</title>
	...
	<body>
		<button onclick="location.href='produits.php'" class="w3-button w3-green">Retour liste produits</button>
			...
			<tbody>
				...
				if(isset($_GET['categorie']))
				{
					$stmt = $pdo->query('SELECT DISTINCT tickets.id, tickets.date 
										...
										where categories.nom ilike \''.$_GET['categorie'].'\' 
										group by tickets.id
				...
				if (isset($_GET['produit']))
				{
					$stmt = $pdo->query('SELECT DISTINCT tickets.id, tickets.date 
										...
										JOIN produits ON (ticket_entry.produit_id = produits.id)
										JOIN categories ON (produits.categorie_id = categories.id)
										where produits.nom ilike \''.$_GET['produit'].'\' 
										group by tickets.id
										order by tickets.date DESC
										LIMIT 3;');
					while($row = $stmt->fetch()):
